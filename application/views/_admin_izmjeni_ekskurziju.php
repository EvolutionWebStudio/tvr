<div class="small-12 columns">
	<h3>Izmjeni ekskurziju</h3>
	<form method="post"  enctype="multipart/form-data" accept-charset="utf-8" action="<?php echo site_url("update_ekskurziju"); ?>">
		...
		<label title="Da li je članak objavljen na sajtu ili je vidljiv samo administratoru.">
			<input type="checkbox" name="status" value="1" <?php if ($param->status) echo "checked"; ?>/> <strong>Objavljena ekskurzija </strong>
		</label>
		</div>
		...
		<div class="small-6 columns">
			...
			<select name="kategorija">
				<?php foreach ($categories as $c): ?>
					<option value="<?php echo $c->kategorija; ?>" <?php if ($c->kategorija == $param->kategorija) echo "selected";  ?>><?php echo $c->kategorija; ?> </option>
				<?php endforeach ?>
			</select>
		...
		<div class="small-12 columns">
			...
			<input type="hidden" name="id" value="<?php echo $param->eid; ?>" /> 
