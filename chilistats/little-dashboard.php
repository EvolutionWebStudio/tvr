...
<div id="analytics-container" class="row">
  ...
  <div class="middle small-6 columns">
    <h3>Podaci o posjećenosti </h3>
	<table width="100%" border="0" cellpadding="5" cellspacing="0" class="summary">
      <tr valign="top">
      <?PHP
		...
		echo "<td width=\"30%\">Posjetilaca</td><td width=\"20%\">$visitors</td>\n";
		echo "<td width=\"30%\">Posjeta</td><td width=\"20%\">$visits</td>\n";
	  ?>
	  </tr>
	  <tr valign="top">
	  <?PHP
	  ...
	  <?PHP
		...
		mysql_free_result($abfrage);
		echo "<td>Odbijeni posjetioci</td><td>" . round(($onepage / $total) * 100, 2) . "%</td>\n";
		// Page/User and 7 days averange
		$from_day = date("Y.m.d", $time - (7 * 24 * 60 * 60));
		$to_day = date("Y.m.d", $time - (24 * 60 * 60));
		// <= ohne heute
		...
		mysql_free_result($abfrage);
		echo "<td>Posjetilaca po stranici</td><td>$page_user</td>\n";
	  ?>
	  </tr>
	  <tr valign="top">
	  <td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
	  ...
	  <?PHP
		...
		mysql_free_result($abfrage);
		echo "<td>Juče (" . date("G:i", $time) . ")</td><td>$yesterday</td>\n";
	  ?>
	  </tr>	
    </table>
  </div>
  ...
  <div class="middle small-6 columns">
	...
	<?PHP
		...
		for ($i = 0; $i < $bar_nr; $i++)
		{
			...
			if (max($bar) > 0)
			{
				$bar_hight = round((170 / max($bar)) * $value);
			}
			else
				$bar_hight = 0;
			if ($bar_hight == 0)
			...
			if ($bar_mark == "$i")
			{
				echo "<td style=\"border-left: #FF0000 1px dotted;\" width=\"19\">";
			}
			else
				echo "<td width=\"19\">";
  ...
  <div class="full small-12 columns">
    <h3>Poslednjih 30 dana </h3>
	...
	<?PHP
		...
		for ($i = 0; $i < $bar_nr; $i++)
		{
			...
			if (max($bar) > 0)
			{
				$bar_hight = round((200 / max($bar)) * $value);
			}
			else
				$bar_hight = 0;
			if ($bar_hight == 0)
			...
			if ($bar_mark == "$i")
			{
				echo "<td style=\"border-left: #FF0000 1px dotted;\" width=\"31\">";
			}
			else
				echo "<td width=\"31\">";
			echo "<div class=\"bar\" style=\"height:" . $bar_hight . "px;\" title=\"" . $bar_title[$i] . " - $value Visitors\"></div></td>\n";
